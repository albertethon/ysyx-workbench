...
#define checkrdr1r2 ;printf("rd:%lx\tsrc1:%lx\tsrc2:%lx\n",R(dest),src1,src2)
 ...
 **/
  ...
  INSTPAT("0000000 ????? ????? 110 ????? 01100 11", or     , I, R(dest) = src1 | src2);//x[rd] = x[rs1] | x[rs2]
  ...
  INSTPAT("??????? ????? ????? 001 ????? 01000 11", sh     , S, Mw(src1 + dest, 2, SEXT(BITS(src2,15,0), 16)));//store halfword:M[x[rs1] + sext(offset)] = x[rs2][15:0]
  INSTPAT("??????? ????? ????? 000 ????? 01000 11", sb     , S, Mw(src1 + dest, 1, SEXT(BITS(src2,7,0), 8)));//store byte:M[x[rs1] + sext(offset)] = x[rs2][7:0]
