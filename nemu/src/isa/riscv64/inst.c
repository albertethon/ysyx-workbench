 ...
 **/
  ...
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, R(dest) = s->snpc; s->dnpc += src1);//x[rd] = pc+4; pc+=sext(offset)
  INSTPAT("??????? ????? ????? 010 ????? 11001 11", jalr   , I, R(dest) = s->snpc; s->dnpc = (src1+src2)&0xfffffffe);//t=pc+4;pc=(x[rs1]+sext(offset))&~1;x[rd]=t
