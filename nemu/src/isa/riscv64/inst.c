...
static void decode_operand(Decode *s, word_t *dest, word_t *src1, word_t *src2, int type) {
  ...
  switch (type) {
    ...
    case TYPE_S: destI(immS(i)); src1R(rs1); src2R(rs2); break;??????? ????? ????? 010 ????? 00000 11
    break;
 ...
 **/
  ...
  INSTPAT("0000000 ????? ????? 111 ????? 01100 11", and    , I, R(dest) = src1 & src2);//x[rd] = sext((x[rs1] + sext(imm))[31:0])
  ...
  INSTPAT("??????? ????? ????? 100 ????? 00000 11", lbu    , I, R(dest) = Mr(src1 + src2, 2));
