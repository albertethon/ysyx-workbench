 ...
 **/
  ...
  INSTPAT("0000000 00001 ????? 010 ????? 00000 11", seqz   , I, R(dest) = (src1!=0)?0:1);//if 0, x[rd]=1, else sltiu rd, rs1, 1
  INSTPAT("??????? ????? ????? 010 ????? 00000 11", sltiu  , I, R(dest) = (src1 < src2)?1:0);//x[rd] = x[rs1] < imm
