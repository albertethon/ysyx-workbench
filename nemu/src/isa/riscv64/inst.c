 ...
 **/
  ...
  INSTPAT("??????? ????? ????? 001 ????? 01000 11", sh     , S, Mw(BITS(src1 + SEXT(dest,32)), 16, BITS(src2,15,0)));//store halfword:M[x[rs1] + sext(offset)] = x[rs2][15:0]
  INSTPAT("??????? ????? ????? 000 ????? 01000 11", sb     , S, Mw(BITS(src1 + SEXT(dest,32)), 8,  BITS(src2,7,0)));//store byte:M[x[rs1] + sext(offset)] = x[rs2][7:0]
