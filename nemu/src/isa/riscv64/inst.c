 ...
 **/
  ...
  INSTPAT("??????? ????? ????? ??? 00000 11011 11", j      , J, R(dest) = s->snpc; s->dnpc = src1 + s->pc);//x[rd] = pc+4; pc+=sext(offset)
  INSTPAT("??????? 00000 ????? 000 ????? 11000 11", beqz   , B, if(src1==0)s->dnpc += dest);//if(rs1==0)pc+=sext(offset)
  INSTPAT("??????? ????? ????? 000 ????? 11000 11", beq    , B, if(src1==src2)s->dnpc += dest);//if(rs1==rs2)pc+=sext(offset)
