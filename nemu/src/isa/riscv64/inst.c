...
=======
  INSTPAT("0000000 ????? ????? 000 ????? 01110 11", addw   , R, R(dest) = SEXT(BITS(src1 + src2,31,0), 31));//x[rd] = sext((x[rs1] + x[rs2])[31:0])
  INSTPAT("??????? ????? ????? 000 ????? 00100 11", addi   , I, R(dest) = src1 + src2);//x[rd] = x[rs1] + sext(imm)
  INSTPAT("??????? ????? ????? 000 ????? 00110 11", addiw  , I, R(dest) = SEXT(BITS(src1 + src2,31,0), 31));//x[rd] = sext((x[rs1] + sext(imm))[31:0])
>>>>>>> refs/remotes/origin/tracer-ysyx2204
...
<<<<<<< HEAD
  ...
  INSTPAT("??????? ????? ????? 110 ????? 11000 11", bltu   , B, if(src1 <  src2) s->dnpc = s->pc + dest);//if(rs1 <_u rs2) pc += sext(offset)
...
=======
  INSTPAT("??????? ????? ????? 101 ????? 11000 11", bge    , B, if(SEXT(src1,64) >= SEXT(src2,64)) s->dnpc = s->pc + dest);//if(rs1 >=_s rs2) pc += sext(offset)
  INSTPAT("??????? ????? ????? 111 ????? 11000 11", bgeu   , B, if(src1 >= src2) s->dnpc = s->pc + dest);//if(rs1 >=_u rs2) pc += sext(offset)
  INSTPAT("??????? ????? ????? 001 ????? 11000 11", bne    , B, if(src1!=src2)s->dnpc = s->pc + dest);//if(rs1!=rs2)pc  +=  sext(offset)
  INSTPAT("0000001 ????? ????? 100 ????? 01100 11", div    , R, R(dest) = SEXT(src1, 64) / SEXT(src2, 64));// x[rd] = x[rs1] /_s x[rs2]
  INSTPAT("0000001 ????? ????? 100 ????? 01110 11", divw   , R, R(dest) = SEXT(BITS(src1, 31, 0), 31) / SEXT(BITS(src2, 31, 0), 31));// x[rd] = sext(x[rs1][31:0] /_s x[rs2][31:0])
  INSTPAT("??????? ????? ????? ??? 00000 11011 11", j      , J, s->dnpc = src1 + s->pc);//pc+=sext(offset)
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, R(dest) = s->snpc; s->dnpc = src1 + s->pc);//x[rd] = pc+4; pc+=sext(offset)
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, R(dest) = s->snpc; s->dnpc = (src1 + src2)&0xfffffffe);//t=pc+4;pc=(x[rs1]+sext(offset))&~1;x[rd]=t
  ...
  INSTPAT("??????? ????? ????? ??? ????? 01101 11", lui    , U, R(dest) = src1);//x[rd] = sext(imm[31:12]<<12)
  INSTPAT("0000001 ????? ????? 000 ????? 01110 11", mulw   , R, R(dest) = SEXT(BITS(src1 * src2, 31, 0), 31));//X[rd] = sext((x[rs1] * x[rs2])[31:0])
>>>>>>> refs/remotes/origin/tracer-ysyx2204
...
<<<<<<< HEAD
  ...
  INSTPAT("0000000 ????? ????? 101 ????? 01110 11", srlw   , R, R(dest) = SEXT(BITS(src1,31,0) >> BITS(src2,4,0), 32));//x[rd] = sext(x[rs1][31:0] >>_u x[rs2][4:0])
=======
  ...
  INSTPAT("??????? ????? ????? 001 ????? 01000 11", sh     , S, Mw(src1 + dest, 2, BITS(src2,15,0)));//store halfword:M[x[rs1] + sext(offset)] = x[rs2][15:0]
  INSTPAT("??????? ????? ????? 000 ????? 01000 11", sb     , S, Mw(src1 + dest, 1, BITS(src2,7,0)));//store byte:M[x[rs1] + sext(offset)] = x[rs2][7:0]
  INSTPAT("010000 ?????? ????? 101 ????? 00100 11", srai   , I, R(dest) = src1 >> (src2 - 0x400));// x[rd] = x[rs1] >>_s shamt
  INSTPAT("000000 ?????? ????? 101 ????? 00100 11", srli   , I, R(dest) = UEXT(src1,64) >> src2);//x[rd] = x[rs1] >>_u shamt
>>>>>>> refs/remotes/origin/tracer-ysyx2204
