...
#define checkrdr1r2 ;printf("rd:%lx\tsrc1:%lx\tsrc2:%lx\tm[%lx]:%lx\n",R(dest),src1,src2,src1+src2,Mr(src1 + src2,4))
 ...
 **/
  ...
  INSTPAT("??????? ????? ????? 001 ????? 01000 11", sh     , S, Mw(src1 + dest, 2, BITS(src2,15,0))checkrdr1r2);//store halfword:M[x[rs1] + sext(offset)] = x[rs2][15:0]
