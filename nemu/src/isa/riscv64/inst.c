...
#define checkrdr1r2 ;printf("rd:%lx\tsrc1:%lxsrc2:%lx",R(dest),src1,src2)
 ...
 **/
  ...
  INSTPAT("??????? ????? ????? 100 ????? 00000 11", lbu    , I, R(dest) = Mr(src1 + src2, 2));//x[rd] = sext(M[x[rs1] + sext(offset)][7:0])
  INSTPAT("??????? ????? ????? 010 ????? 00000 11", lw     , I, R(dest) = SEXT(BITS(Mr(src1 + src2, 4), 31, 0), 31);printf("rd:%lx\tsrc1:%lxsrc2:%lx",R(dest),src1,src2));//load word:x[rd] = sext(M[x[rs1] + sext(offset)][31:0])
